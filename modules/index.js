            ...
            "action": function (messageChatId) {
                ...
                request({ url: url, encoding: null }, function (error, response, body) {
                    if (!error) {
                        ...
                        bot.sendMessage(messageChatId, html_joke);
                    } else {

                        bot.sendMessage(messageChatId, "Произошла ошибка: " + error);
                    }
                });
                //return "Нет.";
            }
            ...
            "action": function (messageChatId) {
                request("http://www.wunderground.com/cgi-bin/findweather/getForecast?&query=47.887403899999995,33.3879114", function (error, response, body) {
                    if (!error) {
                        ...
                        bot.sendMessage(messageChatId, "Текущая температура " + temperature + " °C, по ощущениям " + feels + " °C");
                    } else {
                        bot.sendMessage(messageChatId, "Произошла ошибка: " + error);
                    }
                });
                ...
                switch (rand_no) {
                    case 1: {
                        request('https://www.goodreads.com/quotes_of_the_day', function (error, response, body) {
                            ...
                            bot.sendMessage(messageChatId, quotes.split('//<!')[0].trim().split('―')[0].trim());
                        });
                    ...
                    case 2: {
                        request('http://www.brainyquote.com/quotes_of_the_day.html', function (error, response, body) {
                            var $ = cheerio.load(body);
                            bot.sendMessage(messageChatId, $('.bqQuoteLink').eq(0).text().trim());
                        });
                    ...
                    case 3: {
                        request('http://www.eduro.com/', function (error, response, body) {
                            var $ = cheerio.load(body);
                            bot.sendMessage(messageChatId, $('.article dailyquote p').eq(0).text().trim());
                        });
            ...
            "action": function (messageChatId, messageText) {
                ...
                request({
                    ...
                    if (random === undefined || random['url'] === undefined) {
                        bot.sendMessage(messageChatId, "Я не нашел ничего подходящего.");
                    } else {
